...
bool try_pop( tbb::concurrent_queue<vector<char>>& q, std::vector<char>& buffer ) { return q.try_pop( buffer ); }
 ...
 */
class queued_chunk_generator : public tbb_shim::filter, boost::noncopyable {
...
// queued_write_task removed as threading is disabled.
 ...
 */
void prt2_particle_ostream::modal_pipeline::add_filter( tbb_shim::filter& filter ) {
...
void prt2_particle_ostream::modal_pipeline::run( size_t maxLiveTokens ) {
    ...
    State oldState = static_cast<State>( m_state );
    if( m_state == Stopped ) {
        m_state = AutomaticRunning;
    } else {
        oldState = static_cast<State>( m_state );
    }
...
bool prt2_particle_ostream::modal_pipeline::step() {
    ...
    // In synchronous mode, we shouldn't block, we just run.
    // But logic implies waiting for 'Stopped' -> 'ManualRunning' transition?
    // Actually manual_run_non_blocking sets it to Manual.

    while( m_state == Stopped ) {
        tbb::this_tbb_thread::yield();
    }

    for( std::vector<tbb_shim::filter*>::iterator i = m_filters.begin(), end = m_filters.end(); i != end; ++i ) {
        tbb_shim::filter& filter = **i;
...
prt2_particle_ostream::prt2_particle_ostream(
    ...
    for( std::vector<boost::shared_ptr<tbb_shim::filter>>::iterator i = m_chunkPipelineFilters.begin(),
                                                                    end = m_chunkPipelineFilters.end();
         i != end; ++i ) {
        m_chunkPipeline->add_filter( **i );
    }

    // THREADING DISABLED: No dummy task, no spawn.
    // We expect the pipeline to run manually via put_particle -> manual_run_non_blocking -> step

    // Sleep for a short period of time, and then make the modal_pipeline manual if it hasn't run.

    // In threaded version, task would be spawned here.
    // Here we just switch to manual mode immediately.
    ...
    if( !m_closeRequested ) {
        ...
        if( m_chunkPipeline->is_manually_running() ) {
            while( m_chunkPipeline->step() ) {
                // Keep stepping to process remaining chunks
            }
        }
...
void prt2_particle_ostream::put_particle( const char* rawParticleData ) {
    ...
    if( currentChunkSize > m_desiredChunkSizeInBytes ) {
        ...
        if( m_chunkPipeline->is_manually_running() ) {
            // In single threaded mode, we MUST process it now because there is no other thread to pick it up.
            // But we only step ONE time?
            // The chunk generator will pick one item.
            // If the filter chain processes one item per step, we should loop?
            // "if !step throw" suggests one step is expected to handle one token?
            // But a pipeline might process multiple.
            // We'll keep original logic: if( !step() ) throw.
