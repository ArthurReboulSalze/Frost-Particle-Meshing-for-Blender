...
void particle_grid_tree::insert_particles_parallel( const frantic::particles::particle_array& pa ) {
    using namespace frantic::graphics;
    using namespace frantic::channels;
    ...
    // Helper struct for parallel sorting
    struct pgt_parallel_insert_helper {
        frantic::graphics::vector3 voxelCoord;
        std::size_t particleIndex;

        // Sort by Z, then Y, then X for locality
        bool operator<( const pgt_parallel_insert_helper& rhs ) const {
            if( voxelCoord.z != rhs.voxelCoord.z )
                return voxelCoord.z < rhs.voxelCoord.z;
            if( voxelCoord.y != rhs.voxelCoord.y )
                return voxelCoord.y < rhs.voxelCoord.y;
            return voxelCoord.x < rhs.voxelCoord.x;
        }
    };

    std::vector<pgt_parallel_insert_helper> sortedIndices( particleCount );

    // Accessors
    channel_accessor<vector3f> posAcc = pa.get_channel_map().get_accessor<vector3f>( _T("Position") );
    channel_accessor<float> radiusAcc;
    bool hasRadius = pa.get_channel_map().has_channel( _T("Radius") );
    if( hasRadius ) {
        radiusAcc = pa.get_channel_map().get_accessor<float>( _T("Radius") );
    }

    // 1. Parallel Collection and Voxel Calculation
    tbb::parallel_for(
        tbb::blocked_range<std::size_t>( 0, particleCount ), [&]( const tbb::blocked_range<std::size_t>& r ) {
            for( size_t i = r.begin(); i != r.end(); ++i ) {
                // accessing m_data directly if possible or using operator[]
                // casting const to ensure char* vs const char* correctness
                const char* particleData = pa[i];
                frantic::graphics::vector3f pos = posAcc.get( particleData );

                bool valid = pos.is_finite();
                if( valid && hasRadius ) {
                    if( radiusAcc.get( particleData ) <= 0 )
                        valid = false;
                }

                if( !valid ) {
                    sortedIndices[i].voxelCoord.z = std::numeric_limits<boost::int32_t>::min();
                    continue;
                }

                sortedIndices[i].voxelCoord = frantic::graphics::vector3::from_floor( m_vcs.get_voxel_coord( pos ) );
                sortedIndices[i].particleIndex = i;
            }
        } );
    ...
    // 3. Skip invalid particles
    auto it = sortedIndices.begin();
    while( it != sortedIndices.end() && it->voxelCoord.z == std::numeric_limits<boost::int32_t>::min() ) {
        ++it;
    }
    ...
    std::size_t validCount = std::distance( it, sortedIndices.end() );

    // 4. Create Leaves and Define Tasks (Sequential)
    struct leaf_task {
        pgt_node_type* leafNode;
        std::size_t startIdx;
        std::size_t endIdx;
        frantic::graphics::vector3 gridStart; // Optimization
    };

    std::vector<leaf_task> tasks;
    // Reserve estimation: unique voxels. Hard to guess, but tasks vector is small.

    frantic::graphics::vector3 currentVoxel = it->voxelCoord;
    std::size_t currentStart = std::distance( sortedIndices.begin(), it );

    // Ensure first leaf exists
    pgt_node_type* currentLeaf = navigate_to_leaf_with_create( currentVoxel );
    if( !currentLeaf->leafData )
        add_new_leaf_data( currentVoxel, currentLeaf );

    for( auto curr = it + 1; curr != sortedIndices.end(); ++curr ) {
        if( curr->voxelCoord.x != currentVoxel.x || curr->voxelCoord.y != currentVoxel.y ||
            curr->voxelCoord.z != currentVoxel.z ) {
            // Push task
            tasks.push_back( { currentLeaf, currentStart, (std::size_t)std::distance( sortedIndices.begin(), curr ),
                               currentLeaf->coord * PGT_SL } );

            currentVoxel = curr->voxelCoord;
            currentStart = std::distance( sortedIndices.begin(), curr );

            currentLeaf = navigate_to_leaf_with_create( currentVoxel );
            if( !currentLeaf->leafData )
                add_new_leaf_data( currentVoxel, currentLeaf );
        }
    }
    // Push final task
    tasks.push_back( { currentLeaf, currentStart, sortedIndices.size(), currentLeaf->coord * PGT_SL } );

    // 5. Parallel Insert into Leaves
    tbb::parallel_for(
        tbb::blocked_range<std::size_t>( 0, tasks.size() ), [&]( const tbb::blocked_range<std::size_t>& r ) {
            for( size_t t = r.begin(); t != r.end(); ++t ) {
                const leaf_task& task = tasks[t];
                pgt_node_type* leaf = task.leafNode;
                const frantic::graphics::vector3& gridStart = task.gridStart;

                for( size_t idx = task.startIdx; idx < task.endIdx; ++idx ) {
                    size_t pIndex = sortedIndices[idx].particleIndex;
                    const char* rawParticleData = pa[pIndex];

                    frantic::graphics::vector3f pos = posAcc.get( rawParticleData );
                    frantic::graphics::vector3 gridCoord =
                        frantic::graphics::vector3::from_floor( m_gcs.get_voxel_coord( pos ) );

                    int gridIndexX = gridCoord.x - gridStart.x;
                    int gridIndexY = gridCoord.y - gridStart.y;
                    int gridIndexZ = gridCoord.z - gridStart.z;

                    if( gridIndexX < 0 )
                        gridIndexX = 0;
                    else if( gridIndexX >= PGT_SL )
                        gridIndexX = PGT_SL - 1;
                    if( gridIndexY < 0 )
                        gridIndexY = 0;
                    else if( gridIndexY >= PGT_SL )
                        gridIndexY = PGT_SL - 1;
                    if( gridIndexZ < 0 )
                        gridIndexZ = 0;
                    else if( gridIndexZ >= PGT_SL )
                        gridIndexZ = PGT_SL - 1;

                    int gridIndex = gridIndexX + ( gridIndexY + gridIndexZ * PGT_SL ) * PGT_SL;

                    leaf->leafData->grid[gridIndex].add_element( rawParticleData, m_channelMap.structure_size() );
                }
            }
        } );

    m_particleCount += validCount;
}
