 ...
 */
class chunk_transposer : public tbb_shim::filter, boost::noncopyable {
    const std::size_t m_particleSize;

  public:
    chunk_transposer( std::size_t particleSize )
        : tbb_shim::filter( false )
...
// zlib compressor.
class zlib_compression : public tbb_shim::filter, boost::noncopyable {
...
#if defined( LZ4_AVAILABLE )
// lz4 compressor.
class lz4_compression : public tbb_shim::filter, boost::noncopyable {
    const frantic::tstring m_streamname;
...
void prt2_writer::get_filters( boost::uint64_t totalParticleCount, frantic::logging::progress_logger& progress,
                               ...
                               std::vector<boost::shared_ptr<tbb_shim::filter>>& outFilters,
    ...
    boost::shared_ptr<tbb_shim::filter> chunkTransposer;
    ...
    boost::shared_ptr<tbb_shim::filter> chunkCompressor;
    switch( compressionScheme ) {
    case prt2_compression_uncompressed:
    case prt2_compression_transpose:
        // No compression.
...
void prt2_writer::write_particle_chunks( boost::shared_ptr<tbb_shim::filter> chunkGenerator,
    ...
    boost::shared_ptr<tbb_shim::pipeline> chunkPipeline( new tbb_shim::pipeline() );

    // Fill the pipeline in with filters.
    chunkPipeline->add_filter( *chunkGenerator.get() );

    std::vector<boost::shared_ptr<tbb_shim::filter>> filters;
    ...
    for( std::vector<boost::shared_ptr<tbb_shim::filter>>::iterator i = filters.begin(), end = filters.end(); i != end;
         ++i ) {
