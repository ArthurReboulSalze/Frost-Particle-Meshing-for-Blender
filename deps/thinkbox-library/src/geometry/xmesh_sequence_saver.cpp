...
#define FRANTIC_DISABLE_THREADS // Disabled for this file due to Boost.Asio build issues
...
void xmesh_sequence_saver::write_xmesh_xml_file( const boost::filesystem::path& filename,
    ...
    vertCount->LinkEndChild( doc.NewText(
        frantic::strings::to_string_classic<std::string>( num_elements( m_cachedGeometryChannel ) ).c_str() ) );
    ...
    faceCount->LinkEndChild( doc.NewText(
        frantic::strings::to_string_classic<std::string>( m_cachedGeometryChannel.m_customFaces.size() /*/ 3*/ )
            .c_str() ) );
    ...
    for( i = m_vertChannelVertDataChanged.begin(); i != m_vertChannelVertDataChanged.end(); ++i ) {
        ...
        dataType->LinkEndChild( doc.NewText(
            frantic::strings::to_string( channel_data_type_str( vertexChannel.m_arity, vertexChannel.m_dataType ) )
                .c_str() ) );
        ...
        if( vertexChannel.m_customFaces.size() > 0 ) {
            ...
            // todo: switch based on whether its a trimesh or not
            faceCount->LinkEndChild( doc.NewText(
                frantic::strings::to_string_classic<std::string>( vertexChannel.m_customFaces.size() /*/ 3*/ )
                    .c_str() ) );
    ...
    for( i = m_faceChannelDataChanged.begin(); i != m_faceChannelDataChanged.end(); ++i ) {
        ...
        dataType->LinkEndChild( doc.NewText(
            frantic::strings::to_string( channel_data_type_str( facesChannel.m_arity, facesChannel.m_dataType ) )
                .c_str() ) );
