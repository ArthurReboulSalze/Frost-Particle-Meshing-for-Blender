...
// #include <tbb/atomic.h> // Disabled
// #include <tbb/task.h>   // Disabled
#include <tbb/task_scheduler_init.h> // Keep this as it was not explicitly commented out in the instruction
...
#pragma warning( disable : 4996 )
    class modal_pipeline {
        ...
        tbb_shim::pipeline m_autoPipeline;        // The tbb pipeline that is used when this is in automatic mode.
        std::vector<tbb_shim::filter*> m_filters; // The filters in the pipeline.
      ...
      public:
        ...
        void add_filter( tbb_shim::filter& filter );
        void clear();
...
#pragma warning( pop )
  ...
  private:
    ...
    std::vector<boost::shared_ptr<tbb_shim::filter>> m_chunkPipelineFilters;
