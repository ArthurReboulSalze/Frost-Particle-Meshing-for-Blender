#pragma once
// This is a public header. Avoid references to cuda or other external
// references.
...
public:
  ...
  /**
   * Access to raw neighbor data for custom CUDA kernels
   */
  const NeighborSet &get_neighbor_set(unsigned int point_set) const {
    return neighbors[point_set];
  }
   ...
   */
  inline unsigned int neighbor(unsigned int point_set, unsigned int i,
   ...
   */
  inline unsigned int *neighbor_list(unsigned int point_set,
...
private:
  ...
  // Implementation and cuda data are hidden in the PointSetImplementation class
  // to avoid unnecessary dependencies in public headers.
  ...
  bool m_dynamic;  // if false the points do not move and the hash values do not
                  // change
  void *m_user_data;
