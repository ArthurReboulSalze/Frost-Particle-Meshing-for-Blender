...
- `GPU / CUDA`: faster Zhu-Bridson-style meshing with a simplified, more stable UI.

Supported Blender version: `5+`
Compatible addon release: `1.23.0+`

---

## Installation

1. Install or symlink the `frost_blender_addon` folder into Blender's addon directory.
2. Open Blender.
3. Enable `Frost Particle Meshing` in Preferences > Add-ons.

If Blender shows `Core Load Error`, the native `.pyd` or one of its bundled `.dll` dependencies failed to load.
...
- `GPU Acceleration`: switches to the CUDA backend.
...
- `blender_frost_adapter.pyd` exists in the addon folder
...
Compatible addon version: `1.23.0+`
